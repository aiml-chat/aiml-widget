var AIML=(()=>{var B=class{constructor(e,a){this.apiUrl=e.replace(/\/$/,""),this.apiKey=a,this._abortController=null}async captureLead(e,a,t){if(!(await fetch(`${this.apiUrl}/v1/leads`,{method:"POST",headers:{"Content-Type":"application/json","X-Api-Key":this.apiKey},body:JSON.stringify({websiteId:e,email:a,question:t,visitorId:null})})).ok)throw new Error("lead capture failed")}abort(){this._abortController&&(this._abortController.abort(),this._abortController=null)}async send(e,a,t,o,i,s={}){this.abort(),this._abortController=new AbortController;let{signal:m}=this._abortController,h=JSON.stringify({message:e,conversationId:t,visitorId:o,history:a.slice(-6),...s.mode?{mode:s.mode}:{},...s.approveActionId?{approveActionId:s.approveActionId,approveAllow:!!s.approveAllow}:{}}),n;try{n=await fetch(`${this.apiUrl}/v1/chat`,{method:"POST",headers:{"Content-Type":"application/json","X-Api-Key":this.apiKey},body:h,signal:m})}catch(S){if(S.name==="AbortError")return;i.onError("network");return}if(n.status===401){i.onError("auth");return}if(n.status===402){i.onError("quota");return}if(n.status===429){let S=n.headers.get("Retry-After")||"60";i.onError("rateLimit",S);return}if(n.status===404){i.onError("noContent");return}if(!n.ok){i.onError("server");return}let p=n.body.getReader(),u=new TextDecoder,v="";try{for(;;){let{done:S,value:_}=await p.read();if(S)break;v+=u.decode(_,{stream:!0});let T=v.split(`
`);v=T.pop();for(let I of T){if(!I.startsWith("data: "))continue;let M=I.slice(6).trim();if(M==="[DONE]"){i.onDone();return}try{let r=JSON.parse(M);r.conversationId&&i.onConversation?.(r.conversationId),r.token!==void 0&&i.onToken(r.token),r.citations&&i.onCitations(r.citations),r.noAnswer&&i.onNoAnswer?.(),r.agent!==void 0&&i.onAgent?.(r.agent),r.handoff&&i.onHandoff?.(r.handoff),r.tool&&i.onTool?.(r.tool),r.confirm&&i.onConfirm?.(r.confirm),r.confirm_result&&i.onConfirmResult?.(r.confirm_result),r.escalate&&i.onEscalate?.()}catch{}}}}catch(S){S.name!=="AbortError"&&i.onError("stream")}}};var D=`:host {
  --aiml-primary: #2563eb;
...
`;function j(g){if(!g)return"";let e=Y(g);e=e.replace(/```(\w*)\n([\s\S]*?)```/g,(m,h,n)=>`<pre><code>${n.trimEnd()}</code></pre>`),e=e.replace(/`([^`]+)`/g,"<code>$1</code>"),e=e.replace(/\*\*(.+?)\*\*/g,"<strong>$1</strong>"),e=e.replace(/__(.+?)__/g,"<strong>$1</strong>"),e=e.replace(/\*([^*\n]+)\*/g,"<em>$1</em>"),e=e.replace(/_([^_\n]+)_/g,"<em>$1</em>"),e=e.replace(/\[([^\]]+)\]\((https?:\/\/[^\)]+)\)/g,'<a href="$2" target="_blank" rel="noopener noreferrer">$1</a>');let a=e.split(`
`),t=[],o=!1,i=!1;for(let m=0;m<a.length;m++){let h=a[m],n=h.match(/^(#{1,3}) (.+)/);if(n){s();let v=Math.min(n[1].length+2,6);t.push(`<h${v}>${n[2]}</h${v}>`);continue}let p=h.match(/^[-*] (.+)/);if(p){o||(t.push("<ul>"),o=!0),t.push(`<li>${p[1]}</li>`);continue}let u=h.match(/^\d+\. (.+)/);if(u){i||(t.push("<ol>"),i=!0),t.push(`<li>${u[1]}</li>`);continue}s(),h.trim()===""?t.push(""):t.push(`<p>${h}</p>`)}s();function s(){o&&(t.push("</ul>"),o=!1),i&&(t.push("</ol>"),i=!1)}return t.join(`
`).replace(/<\/p>\n<p>/g,"<br>").replace(/\n{2,}/g,`
`)}function Y(g){return g.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}var w={chat:'<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>',close:'<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>',send:'<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>',bot:"\u{1F916}",minus:'<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><line x1="5" y1="12" x2="19" y2="12"/></svg>',maximize:'<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 3 21 3 21 9"/><polyline points="9 21 3 21 3 15"/><line x1="21" y1="3" x2="14" y2="10"/><line x1="3" y1="21" x2="10" y2="14"/></svg>',restore:'<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><polyline points="4 14 10 14 10 20"/><polyline points="20 10 14 10 14 4"/><line x1="14" y1="10" x2="21" y2="3"/><line x1="3" y1="21" x2="10" y2="14"/></svg>',menu:'<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round"><line x1="4" y1="7" x2="20" y2="7"/><line x1="4" y1="12" x2="20" y2="12"/><line x1="4" y1="17" x2="20" y2="17"/></svg>'},k=g=>`<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round">${g}</svg>`,x={msg:k('<path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/>'),spark:k('<path d="M12 3v4M12 17v4M3 12h4M17 12h4M6 6l2 2M16 16l2 2M18 6l-2 2M8 16l-2 2"/>'),code:k('<polyline points="16 18 22 12 16 6"/><polyline points="8 6 2 12 8 18"/>'),arrow:k('<line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/>'),shield:k('<path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/>'),lock:k('<rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/>'),check:k('<polyline points="20 6 9 17 4 12"/>'),x:k('<circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/>'),loader:k('<line x1="12" y1="2" x2="12" y2="6"/><line x1="12" y1="18" x2="12" y2="22"/><line x1="4.9" y1="4.9" x2="7.8" y2="7.8"/><line x1="16.2" y1="16.2" x2="19.1" y2="19.1"/><line x1="2" y1="12" x2="6" y2="12"/><line x1="18" y1="12" x2="22" y2="12"/><line x1="4.9" y1="19.1" x2="7.8" y2="16.2"/><line x1="16.2" y1="7.8" x2="19.1" y2="4.9"/>'),user:k('<path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/>')},K={Support:{accent:"#4F46E5",icon:"msg"},Sales:{accent:"#7C3AED",icon:"spark"},Technical:{accent:"#0F766E",icon:"code"}},W={accent:"#78716C",icon:"spark"},H=g=>K[g]||W,N=class{constructor(e){this.config=e,this.host=null,this.shadow=null,this.isOpen=!1,this._streaming=!1,this._streamingEl=null,this._streamBuffer="",this._currentBotMsg=null,this._lastAgent=null,this._confirmCards={},this._toolGroup=null,this._toolRows={}}mount(){this.host=document.createElement("div"),this.host.setAttribute("id","aiml-widget-host"),this.host.setAttribute("data-theme",this.config.theme||"auto"),document.body.appendChild(this.host),this.shadow=this.host.attachShadow({mode:"open"});let e=document.createElement("style");e.textContent=D,this.shadow.appendChild(e);let a={none:"0px",md:"12px",xl:"20px"},t=[];this.config.primaryColor&&t.push(`--aiml-primary: ${this.config.primaryColor};`),this.config.radius in a&&t.push(`--aiml-radius: ${a[this.config.radius]};`),t.push(`--aiml-offset-x: ${this.config.offsetX??24}px;`),t.push(`--aiml-offset-y: ${this.config.offsetY??24}px;`),this.config.zIndex&&t.push(`--aiml-z: ${this.config.zIndex};`),e.textContent+=`:host { ${t.join(" ")} }`,this.shadow.appendChild(this._buildTrigger()),this.config.launcherLabel&&this.shadow.appendChild(this._buildLauncherLabel()),this.shadow.appendChild(this._buildWindow()),this._bindEvents(),this._focusTrap()}_buildTrigger(){let e=document.createElement("button"),a=this.config.launcherSize==="sm"?" aiml-sz-sm":this.config.launcherSize==="lg"?" aiml-sz-lg":"";e.className=`aiml-trigger${this.config.position==="left"?" aiml-left":""}${a}`,e.setAttribute("aria-label","Open AI chat assistant"),e.setAttribute("aria-expanded","false"),e.setAttribute("aria-controls","aiml-chat-window");let t=this.config.launcherIconUrl?`<img class="aiml-trigger-img" src="${L(this.config.launcherIconUrl)}" alt="" />`:w.chat;return e.innerHTML=`
      <span class="aiml-icon-open" aria-hidden="true">${t}</span>
      <span class="aiml-icon-close" aria-hidden="true">${w.close}</span>`,e}_buildLauncherLabel(){let e="aiml_label_dismissed",a=!1;try{a=sessionStorage.getItem(e)==="1"}catch{}let t=document.createElement("div");return a||(t.className=`aiml-launcher-label${this.config.position==="left"?" aiml-left":""}`,t.innerHTML=`<span>${f(this.config.launcherLabel)}</span><button class="aiml-label-dismiss" aria-label="Dismiss">${w.close}</button>`,t.querySelector("span").addEventListener("click",()=>this.open()),t.querySelector(".aiml-label-dismiss").addEventListener("click",o=>{o.stopPropagation(),t.remove();try{sessionStorage.setItem(e,"1")}catch{}}),this._labelEl=t),t}_buildWindow(){let e=document.createElement("div");e.className=`aiml-window aiml-hidden${this.config.position==="left"?" aiml-left":""}`,e.setAttribute("id","aiml-chat-window"),e.setAttribute("role","dialog"),e.setAttribute("aria-label","AI Chat Assistant"),e.setAttribute("aria-modal","false");let a=this.config.avatarUrl?`<img class="aiml-avatar-img" src="${L(this.config.avatarUrl)}" alt="" />`:w.bot,t=!!this.config.agentMode,o=H("Support").accent,i=this.config.title||(t?"AI agent team":"AI Assistant"),s=t?'Now: <span class="aiml-header-now" style="color:#fff">Support</span>':f(this.config.subtitle||"Ask me anything");return e.innerHTML=`
      <div class="aiml-header">
        ...
        <div class="aiml-header-info">
          ...
          <div class="aiml-header-subtitle">${s}</div>
        </div>
        <span class="aiml-label" title="You are interacting with an AI">AI</span>
        ${t?`<button class="aiml-hbtn aiml-menu-btn" aria-label="More options" aria-haspopup="true" aria-expanded="false" title="More">${w.menu}</button>`:""}
        <button class="aiml-hbtn aiml-max-btn" aria-label="Full screen" title="Full screen">${w.maximize}</button>
        <button class="aiml-hbtn aiml-close-btn" aria-label="Minimize chat" title="Minimize">${w.minus}</button>
      ...
      <p class="aiml-lead-success aiml-hidden" role="status">Thanks! We'll be in touch.</p>`;let i=o.querySelector(".aiml-lead-email"),s=o.querySelector(".aiml-lead-submit"),m=o.querySelector(".aiml-lead-error"),h=o.querySelector(".aiml-lead-success");s.addEventListener("click",async()=>{let n=i.value.trim();if(!n||!n.includes("@")){m.classList.remove("aiml-hidden");return}m.classList.add("aiml-hidden"),s.disabled=!0;try{await a(n,e),i.classList.add("aiml-hidden"),s.classList.add("aiml-hidden"),h.classList.remove("aiml-hidden")}catch{s.disabled=!1,m.textContent="Failed to submit. Please try again.",m.classList.remove("aiml-hidden")}}),t.appendChild(o),this._scrollToBottom(),setTimeout(()=>i.focus(),50)}_scrollToBottom(){let e=this.shadow.querySelector(".aiml-messages");e&&(e.scrollTop=e.scrollHeight)}};function f(g){return String(g).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function L(g){return String(g||"").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}(function(){"use strict";if(window.__aimlWidgetLoaded)return;window.__aimlWidgetLoaded=!0;let g="Hi! \u{1F44B} I'm this site's AI assistant. Ask me anything \u2014 I'll answer from this site's own content and show you the sources.",e=document.currentScript||document.querySelector("script[data-api-key]");if(!e){console.warn("[AIML] No script tag found with data-api-key attribute.");return}let a=e.getAttribute("data-api-key"),t=e.getAttribute("data-website-id"),o=e.getAttribute("data-api-url")||"https://api.aiml.chat",i=r=>e.getAttribute("data-"+r),s=i("suggested-questions"),m=s?s.split("|").map(r=>r.trim()).filter(Boolean):null;if(!a){console.warn("[AIML] Missing data-api-key on script tag.");return}let h=r=>r&&/^(#[0-9a-fA-F]{3,8}|(rgb|hsl)a?\([\d\s.,%\/]+\))$/.test(r.trim())?r.trim():null,n=r=>r&&/^https:\/\/[^\s"'<>]+$/i.test(r.trim())?r.trim():null,p=(r,c,O,b)=>{let l=parseInt(r,10);return Number.isFinite(l)?Math.min(b,Math.max(O,l)):c},u=(r,c)=>c.includes(r)?r:null,v=`aiml_session_${t||"default"}`;function S(){try{let r=sessionStorage.getItem(v);return r?JSON.parse(r):{conversationId:null,history:[],visitorId:T()}}catch{return{conversationId:null,history:[],visitorId:T()}}}function _(r){try{sessionStorage.setItem(v,JSON.stringify(r))}catch{}}function T(){return"v-"+Math.random().toString(36).slice(2)+Date.now().toString(36)}async function I(){try{let r=await fetch(`${o}/v1/widgets/${encodeURIComponent(a)}/config`,{headers:{"X-Api-Key":a}});if(r.ok)return await r.json()}catch{}return{}}async function M(){let r=await I(),c=S(),O=m??r.suggestedQuestions??[],b={position:u(i("position"),["left","right"])||r.position||"right",theme:u(i("theme"),["light","dark","auto"])||r.theme||"auto",primaryColor:h(i("primary-color"))||h(r.primaryColor)||null,title:i("title")||r.title||"AI Assistant",subtitle:i("subtitle")||r.subtitle||"Ask me anything",placeholder:r.placeholder||"Ask a question\u2026",greeting:i("greeting")||r.greeting||null,showBranding:r.showBranding!==!1,suggestedQuestions:O,avatarUrl:n(i("avatar"))||n(r.avatarUrl)||null,launcherIconUrl:n(i("launcher-icon"))||n(r.launcherIconUrl)||null,launcherSize:u(i("launcher-size"),["sm","md","lg"])||r.launcherSize||"md",launcherLabel:i("launcher-label")??r.launcherLabel??null,radius:u(i("radius"),["none","md","xl"])||r.radius||"md",offsetX:p(i("offset-x")??r.offsetX,24,0,400),offsetY:p(i("offset-y")??r.offsetY,24,0,400),zIndex:p(i("z-index")??r.zIndex,2147483647,1,2147483647),autoOpenDelaySec:p(i("auto-open")??r.autoOpenDelaySec,0,0,600),hideOnMobile:(i("hide-mobile")??String(r.hideOnMobile??!1))==="true",agentMode:i("mode")==="agent"||r.assistantMode==="agent",showAgentName:(i("show-agent-name")??String(r.showAgentName??!0))!=="false"};if(b.hideOnMobile&&window.matchMedia("(max-width: 640px)").matches)return;let l=new N(b),E=new B(o,a);l.mount(),l.showGreeting(b.greeting||g),l.showSuggestedChips(b.suggestedQuestions,"Try asking:"),l.onTalkToHuman=t?()=>l.showLeadCaptureForm("",(z,y)=>E.captureLead(t,z,y)):null,l.onClear=()=>{c.conversationId=null,c.history=[],_(c),l.showGreeting(b.greeting||g),l.showSuggestedChips(b.suggestedQuestions,"Try asking:")},b.autoOpenDelaySec>0&&!c.autoOpened&&setTimeout(()=>{l.isOpen||l.open(),c.autoOpened=!0,_(c)},b.autoOpenDelaySec*1e3),l.host.addEventListener("aiml:send",async z=>{let y=z.detail.text;l.hideWelcomeState(),c.history.push({role:"user",content:y}),_(c),l.appendUserMessage(y),l.startBotMessage();let A="",q=!1,$=d=>{q||(q=!0,l.finishBotMessage(d),A&&(c.history.push({role:"assistant",content:A}),_(c)))};await E.send(y,c.history.slice(0,-1),c.conversationId,c.visitorId,{onConversation(d){d&&c.conversationId!==d&&(c.conversationId=d,_(c))},onToken(d){A+=d,l.appendToken(d)},onCitations(d){$(d)},onAgent(d){l.setAgent(d)},onHandoff(d){l.showHandoff(d)},onTool(d){l.showTool(d)},onConfirm(d){l.showConfirmCard(d,C=>U(d.id,C))},onConfirmResult(d){l.showConfirmResult(d)},onEscalate(){t&&l.showEscalate(()=>l.showLeadCaptureForm(y,(d,C)=>E.captureLead(t,d,C)))},onNoAnswer(){$([]),l.showNoAnswerHelp(y,b.suggestedQuestions,t?(d,C)=>E.captureLead(t,d,C):null)},onDone(){$([])},onError(d,C){if(d==="noContent"&&t){l.showLeadCaptureForm(y,async(R,G)=>{await E.captureLead(t,R,G)});return}let P={auth:"Authentication failed. Please check your API key.",quota:"Monthly message quota reached. Please upgrade your plan.",rateLimit:`Too many requests. Please wait ${C||60} seconds.`,noContent:"No answer found. Please contact us directly.",network:"Connection error. Please check your network and try again.",stream:"Stream interrupted. Please try again.",server:"Server error. Please try again later."};l.showStatus("error",P[d]||"Something went wrong. Please try again.")}},{mode:b.agentMode?"agent":void 0})});async function U(z,y){l.startBotMessage();let A="",q=!1,$=()=>{q||(q=!0,l.finishBotMessage([]),A&&(c.history.push({role:"assistant",content:A}),_(c)))};await E.send("",c.history,c.conversationId,c.visitorId,{onToken(d){A+=d,l.appendToken(d)},onCitations(){$()},onConfirmResult(d){l.showConfirmResult(d)},onDone(){$()},onError(){l.showStatus("error","Could not complete that action. Please try again.")}},{mode:"agent",approveActionId:z,approveAllow:y})}window.AIML={open:()=>l.open(),close:()=>l.close(),toggle:()=>l.toggle(),clearHistory:()=>{c.history=[],c.conversationId=null,_(c)}}}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",M):M()})();})();
