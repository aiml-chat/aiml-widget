...
`);b=L.pop();for(let q of L){if(!q.startsWith("data: "))continue;let M=q.slice(6).trim();if(M==="[DONE]"){i.onDone();return}try{let r=JSON.parse(M);r.conversationId&&i.onConversation?.(r.conversationId),r.token!==void 0&&i.onToken(r.token),r.citations&&i.onCitations(r.citations),r.noAnswer&&i.onNoAnswer?.(),r.agent!==void 0&&i.onAgent?.(r.agent),r.handoff&&i.onHandoff?.(r.handoff),r.tool&&i.onTool?.(r.tool),r.confirm&&i.onConfirm?.(r.confirm),r.confirm_result&&i.onConfirmResult?.(r.confirm_result),r.escalate&&i.onEscalate?.()}catch{}}}}catch(w){w.name!=="AbortError"&&i.onError("stream")}}};var U=`:host {
  --aiml-primary: #2563eb;
